...
class MonthReport(UserPassesTestMixin, View):
    login_url = '/login'
    ...
    def get(self, request):
        ...
        pdf.setTitle(f'month-report-{current_date.month}-{current_date.year}')
        pdf.setFont('Helvetica-Oblique', 14)
        pdf.drawString(220, 820, f'Relatório do mês {current_date.month} de {current_date.year}')
       
        payment_method = Sale.objects.filter(datetime__month=current_date.month).values('payment_method').annotate(count=Count('payment_method')).order_by('-count').first()
        pdf.setFont('Helvetica', 12)
        if payment_method != None:
            pdf.drawString(10, 790, f'Método de pagamento mais utilizado: {payment_method["payment_method"]}, em {payment_method["count"]} vendas.')

        sold_amount = sum([product.quantity for product in Product_Sale.objects.filter(sale__datetime__month=current_date.month)])
        pdf.drawString(10, 770, f'Foram vendidos: {sold_amount} produtos.')

        total_sales_value = Sale.objects.filter(datetime__month=current_date.month).aggregate(Count('id'), Sum('total'))
        pdf.drawString(10, 750, f'Valor total de vendas: R${total_sales_value["total__sum"]:.2f}')
        ...
        for sale in Sale.objects.filter(datetime__month=current_date.month):
            if y <= 40:
                pdf.showPage()
                y = 790
            pdf.setFont('Helvetica', 10)
        ...
        return FileResponse(buffer, as_attachment=True, filename=f'month-report-{current_date.month}-{current_date.year}.pdf')
